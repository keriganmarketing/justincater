<!-- Responsive Template - Created by: Bryan -->
...
<div 
    tabindex="0" 
    class="cycloneslider cycloneslider-template-responsive-img <?php echo esc_attr( $slider_html_id ); ?>"
...
>
    ...
    <script>
        ( function($) {
            var ctri_id = '#' + <?php echo json_encode( esc_attr( $slider_html_id ) ); ?>;

            function ctri_scale() {
                var ctri_setting_h = (<?php echo json_encode( esc_attr( $slider_settings['auto_height'] ) ); ?>).split( ':' );
                var ctri_w = ctri_setting_h[0];
                var ctri_h = ctri_setting_h[1];
                var ctri_s  = $( ctri_id ).width();
                var ctri_nh = Math.floor( ctri_h * ctri_s / ctri_w );
                $( ctri_id ).css({ height: ctri_nh });
            }
            ctri_scale();
    ...
    ( function() {
        ...
        /* Reset captions */
        slideshow.find(".cycloneslider-slide").find(".cycloneslider-caption-title, .cycloneslider-caption-description").delay(200).animate({paddingBottom:0,opacity:0},0);
        
        /* Animate captions */
        slideshow.on( 'cycle-before', function(event, optionHash, outgoingSlideEl, incomingSlideEl, forwardFlag) {
            jQuery(incomingSlideEl).find(".cycloneslider-caption-title, .cycloneslider-caption-description").delay(1000).animate({paddingBottom:100,opacity:1},500);
        });
        ...
        slideshow.find(".cycloneslider-slide").eq(0).find(".cycloneslider-caption-title, .cycloneslider-caption-description").delay(1000).animate({paddingBottom:100,opacity:1},500);
