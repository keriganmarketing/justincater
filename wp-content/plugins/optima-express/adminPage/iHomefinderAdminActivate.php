	...
	protected function getContent() {
		...
		<?php } elseif($section === "free-trial") { ?>
			...
			if(count($errors) === 0) {
				...
				$requestUrl = "http://" . iHomefinderConstants::IHOMEFINDER_STORE_EXTERNAL_URL . "/optima-express-trial.php";
				set_time_limit(90);
				$requestArgs = array(
					"timeout" => "90",
					"body" => $params
						...
						<tr>
							...
							<td>
								<select id="marketBoost" name="marketBoost">
									<option value="true" <?php if($marketBoost) { ?> selected="selected" <?php } ?>>Optima Express + MarketBoost</option>
									<option value="false" <?php if(!$marketBoost) { ?> selected="selected" <?php } ?>>Optima Express</option>
								</select>
							</td>
						</tr>
						<tr>
